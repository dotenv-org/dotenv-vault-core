...
function parseVault(options) {
  ...
  try {
    // Decrypt
    const attrs = _instructions(result, _dotenvKey())
    decrypted = decrypt(attrs.ciphertext, attrs.key)

  } catch (error) {
    if (_dotenvKey2().length === 0) {
      throw error
    }

    // Decrypt with DOTENV_KEY2 if set
    const attrs = _instructions(result, _dotenvKey2())
    decrypted = decrypt(attrs.ciphertext, attrs.key) // Decrypt payload
  }
