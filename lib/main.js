...
// for use with key rotation
function _dotenvKey2() {
  if (process.env.DOTENV_KEY2 && process.env.DOTENV_KEY2.length > 0) {
    return process.env.DOTENV_KEY2
  }
...
function parseVault(options) {
  ...
  const instructions = _instructions(result, _dotenvKey())
  const ciphertext = instructions.ciphertext
  const key = instructions.key

  // Decrypt payload
  const decrypted = decrypt(ciphertext, key) // TODO: handle when this fails and then iterate to DOTENV_KEY2
...
function config(options) {
  ...
  if (_dotenvKey().length === 0 && _dotenvKey2().length === 0) {
